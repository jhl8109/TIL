...
def makeTable(index1, index2):
    if (index1 == index2): 
        ...
        for i in range(0,gap) :
            ...
            absBig = min(absBig, calculator(dpMax[index1][index1 + i], dpMax[index1 + i + 1][index2], tokens[(index1 + i) * 2]));                absBig = min(absBig, calculator(dpMin[index1][index1 + i], dpMin[index1 + i + 1][index2], tokens[(index1 + i) * 2]));
...
makeTable(1,int(N/2));
#print(tokens);
#for i in range(0,len(dpMax)) :
#    print(dpMax[i]);
#print("\n");  
#for i in range(0,len(dpMin)) :
#    print(dpMin[i]);
#print("\n");      
